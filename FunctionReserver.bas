...
'---------Client nombre de voiture en possesion
If GetNbreVoitEnPossession(Trim(FrmReserver.ComboNumID)) > 0 Then
  x = MsgBox("Ce Client a déja Réserver " & GetNbreVoitEnPossession(Trim(FrmReserver.ComboNumID)) & " Voitures , Continuer ?", vbInformation + vbYesNo, "Gestion Location de Voitures")
  If x = vbYes Then
debut:
     If GetClientStatut(Trim(FrmReserver.ComboNumID)) <> "Interdit" Then
       ...
       Else         'si la voiture n existe pas l'ajouter
        ...
        End If
    Else
      MsgBox "Ce Client N'est Pas Autoriser de Reserverver Une Voiture", vbInformation + vbOKOnly, "Gestion Location de Voitures"
    End If
  Else
    Unload FrmReserver
  End If
...
Function OpenModRes()
FrmReserver.CmdImprimer.Visible = True
debut:
...
x = DataTab(1)
AddEvent ("Réservation Ajouter: " & DataTab(0) & ", ClientID = " & DataTab(1) & ", Mat = " & MatID)
AddClientFid (x)
...
Call LoadDbReservData(UCase(FrmReserver.TextResID.Text))
    ...
    If FrmReserver.OptionAuGar.Value = True Then
            DataTab(9) = "Au Garage"
            If Not IsDate(DataTab(10)) Then
              DataTab(10) = "Non Rendu"
            End If
            If Not IsDate(DataTab(11)) Then
              DataTab(11) = "Non Donnée au Client"
            End If
