...
'-----------------------------------------
 ...
 If FrmUser.ComboType <> "" Then
   If FrmUser.TextNom.Text <> "" Then
     If FrmUser.TextPrenom.Text <> "" Then
       If FrmUser.TextEmail.Text <> "" Then
         If Len(FrmUser.TextPass.Text) >= 4 Then
           If Trim(FrmUser.TextComfirmPass.Text) = (FrmUser.TextPass.Text) Then
              ChampsAddUserOk = True
           Else
              MsgBox "Mot de passe de confirmation Incorrect !!", vbInformation + vbOKOnly, "Gestion Location de Voitures"
              FrmUser.TextComfirmPass.Text = ""
              FrmUser.TextPass.Text = ""
              FrmUser.TextPass.SetFocus
           End If
         Else
            MsgBox "Mot de passe très court !!", vbInformation + vbOKOnly, "Gestion Location de Voitures"
            FrmUser.TextPass.SetFocus
         End If
       Else
          MsgBox "Adresse Email Vide !!", vbInformation + vbOKOnly, "Gestion Location de Voitures"
          FrmUser.TextEmail.SetFocus
       End If
     Else
        MsgBox "Prénom Vide !!", vbInformation + vbOKOnly, "Gestion Location de Voitures"
        FrmUser.TextPrenom.SetFocus
     End If
   Else
      MsgBox "Nom Vide !!", vbInformation + vbOKOnly, "Gestion Location de Voitures"
      FrmUser.TextNom.SetFocus
   End If
 Else
    MsgBox "Choisir Type Utilisateur !!", vbInformation + vbOKOnly, "Gestion Location de Voitures"
    FrmUser.ComboType.SetFocus
 End If
...
Function LoadFilledUserData()
    DataTab(0) = Trim(FrmUser.TextEmail.Text)
    DataTab(1) = Trim(FrmUser.TextPass.Text)
    DataTab(2) = FrmUser.ComboType
