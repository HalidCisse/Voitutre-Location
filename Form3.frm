...
Begin VB.Form Main 
   ...
   Begin VB.CommandButton CmdCherRes 
      ...
      Style           =   1  'Graphical
      TabIndex        =   8
      ToolTipText     =   "Click pour chercher un reservation"
      Top             =   2400
      Width           =   1695
   End
   Begin VB.CommandButton CmdListRes 
      ...
      Style           =   1  'Graphical
      TabIndex        =   7
      ToolTipText     =   "Click pour voir la list des reservations"
      Top             =   4320
      Width           =   1695
   End
   ...
   Begin VB.CommandButton CmdRes 
      ...
      Style           =   1  'Graphical
      TabIndex        =   6
      ToolTipText     =   "Click pour enrégistrer un reservatiion"
      Top             =   360
      Width           =   1695
   End
...
If UserType = "ADMIN" Then
  Call RemplirGrid(FrmShowEvents, FrmShowEvents.EventsGrid, "SELECT Message,UserEmail,EventTime,EventDate FROM EVENTS ORDER BY EventDate DESC, EventTime DESC")
Else
  Call RemplirGrid(FrmShowEvents, FrmShowEvents.EventsGrid, "SELECT Message,UserEmail,EventTime,EventDate FROM EVENTS WHERE UserEmail = '" & UserEmail & "' ORDER BY EventDate DESC, EventTime DESC")
End If
End Sub
                           '----------------------------
Private Sub mnResDPAss_Click()
  Call RemplirGridResS("SELECT * FROM RESERVATIONS WHERE ResStatut = 'Avec Client' AND Cdate(ResFin) < Date()")
End Sub

Private Sub mnResNCpay_Click()
  Call RemplirGridResS("SELECT * FROM RESERVATIONS WHERE RestAPayer > '0' ")
End Sub

Private Sub mnResTerm_Click()
  Call RemplirGridResS("SELECT * FROM RESERVATIONS WHERE ResStatut = 'Rendu' AND RestAPayer <= '0'")
End Sub
...
Private Sub mnResEnCours_Click()
  Call RemplirGridResS("SELECT * FROM RESERVATIONS WHERE ResStatut = 'Avec Client'")
End Sub

'                  ----------------------------------------

Private Sub mnClientAvecVoit_Click()
   Call RemplirGridClientS("SELECT * From ClIENTS INNER JOIN RESERVATIONS ON CLIENTS.NumID=RESERVATIONS.NumID WHERE RESERVATIONS.ResStatut = 'Avec Client'")
End Sub
...
Private Sub CmdListRes_Click()
    Call RemplirGrid(FrmShowRes, FrmShowRes.ResGrid, "select * from RESERVATIONS")
End Sub
...
Private Sub CmdListClient_Click()
   Call RemplirGrid(FrmShowClient, FrmShowClient.ClientGrid, "select * from CLIENTS")
End Sub
...
Private Sub CmdListVoit_Click()
   Call RemplirGrid(FrmShowVoit, FrmShowVoit.VoitGrid, "select * from VOITURE")
End Sub
'##############################################################################
