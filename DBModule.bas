...
Function EffaceOldEvent(Mois As String)
  Dim rs As New ADODB.Recordset
    SQL = "SELECT * FROM EVENTS WHERE Datediff('m', Cdate(EventDate),Date()) > '" & Mois & "'  "
    rs.Open SQL, CN, adOpenKeyset
        If Not rs.EOF Then
           CN.Execute ("DELETE FROM EVENTS WHERE Datediff('m', Cdate(EventDate),Date()) > '" & Mois & "'")
        End If
 rs.Close
 Set rs = Nothing
End Function
...
'cette fonction verifie si la vignette est valide sinon elle modifie le statut en vignette invalide
    ...
    rs.Open SQL, CN, adOpenKeyset
      For i = 0 To rs.RecordCount - 1
        ...
        MsgBox "La Vignette de la voiture [" & rs(i) & "] a Expirée", vbInformation + vbOKOnly, "Gestion Location de Voitures"
        AddEvent ("[AUTO] Satut de [" & rs(i) & "] -> Vignette Invalide")
      Next i
    rs.Close
    Set rs = Nothing
End Function
...
'cette fonction verifie si l'assurance est valide sinon elle modifie le statut en assurance invalide
    ...
    rs.Open SQL, CN, adOpenKeyset
      For i = 0 To rs.RecordCount - 1
        ...
        MsgBox "L'Assurance de la voiture [" & rs(i) & "] a Expirée", vbInformation + vbOKOnly, "Gestion Location de Voitures"
        AddEvent ("[AUTO] Satut de [" & rs(i) & "] -> Assurance Invalide")
      Next i
    rs.Close
    Set rs = Nothing
End Function
  ...
  Dim rs As New ADODB.Recordset
    ...
    rs.Open SQL, CN, adOpenKeyset
      For i = 0 To rs.RecordCount - 1
        ...
        MsgBox "L' Age de la voiture [" & rs(i) & "] a Expirée", vbInformation + vbOKOnly, "Gestion Location de Voitures"
        AddEvent ("[AUTO] Satut de [" & rs(i) & "] -> Indisponible")
      Next
    rs.Close
    Set rs = Nothing
End Function
